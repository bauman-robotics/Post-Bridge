...
## 🔑 Управление сессиями (Conversation ID)

Система поддерживает **продолжение диалогов** с помощью `conversation_id`, который сохраняется между письмами.

### Как это работает

1. **Первое письмо** (без SID) — создается новая сессия
2. **Ответ DeepSeek** содержит `conversation_id` (формат: `uuid:номер`)
3. **ID сохраняется** в `data/sessions.json` для отправителя
4. **Следующие письма** с `[SID:...]` в теме продолжают диалог

### Формат SID в теме письма

[SID:4d11613e-6e3f-43a2-bec0-8d203136d4da:2] deepseek 

- `4d11613e-6e3f-43a2-bec0-8d203136d4da` — UUID сессии
- `:2` — номер сообщения в сессии (увеличивается автоматически)
...
Особенности
    ...
    🔄 ID обновляется при каждом запросе (:2 → :4 → :6)
    💾 Все сессии сохраняются (до 20 последних на пользователя)
    🚫 Без SID в теме — создается новая сессия
    📧 SID в теме — продолжается существующая сессия
